...
const Generator: React.FC<GeneratorProps> = ({ dropdownOptions1, dropdownOptions2, responseReceived }) => {
  const defaultTextValue = '';

  const [dropdownValue1, setDropdownValue1] = useState<string>(dropdownOptions2.length > 0 ? dropdownOptions2[0].value : '');
  const [dropdownValue2, setDropdownValue2] = useState<string>(dropdownOptions2.length > 0 ? dropdownOptions2[0].value : '');
  ...
  return (
    <div className="generator-container">
      ...
      <button className="generate-button" onClick={handleGenerate} disabled={!responseReceived}>Сгенерировать</button>
